 ...
 * @author dbdtoan
 */
@WebServlet(name = "assignmentSubmit", urlPatterns = {"/assignmentSubmit"})
public class AssignmentSubmitServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AssignmentSubmitServlet</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AssignmentSubmitServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (assignmentIdParam != null) {
            try {
                ...
                List<Submit> submissions = assignmentDao.getSubmitsByAssId(assignmentId);
                request.setAttribute("submissions", submissions);
                request.getRequestDispatcher("assignSubmit.jsp").forward(request, response);
