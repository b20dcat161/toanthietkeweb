 ...
 * @author dbdtoan
 */
@WebServlet(name="uploadChallenge", urlPatterns={"/uploadChallenge"})
   ...
   private final ChallengeDao challengeDao = new ChallengeDao();
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet UploadChallengeServlet</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet UploadChallengeServlet at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        ...
        if ("teacher".equals(SessionUtils.getRole(request))) {
            ...
            String filename = FileUtils.uploadFile(request,
                    getServletContext().getRealPath("/"+"challenges"+File.separator),
                    "file",
                    false);
            System.out.println("filename:"+ filename);
            if(filename != null){
                challengeDao.createChallenge(filename, hint);
            }
